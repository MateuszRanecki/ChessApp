...
namespace Chess_App
{
    internal sealed class ChessBoard
    {
        ...
        internal VariableSquare[] Squares;   
        internal int Score;
        ...
        internal ChessMove LastMove;

        internal byte WhiteKingPosition;
        internal byte BlackKingPosition;

        internal bool[] BlackAttackedSquares;
        internal bool[] WhiteAttackedSquares;
        ...
        internal ChessBoard(string fen) : this()
        {
            byte index = 0;
            byte spc = 0;
            ...
            foreach (char c in fen)
            {
                ...
                if (index < 64 && spc == 0)
                {
                    ...
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Pawn, ChessPieceColor.White);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'N')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Knight, ChessPieceColor.White);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'B')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Bishop, ChessPieceColor.White);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'R')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Rook, ChessPieceColor.White);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'Q')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Queen, ChessPieceColor.White);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'K')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.King, ChessPieceColor.White);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'p')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Pawn, ChessPieceColor.Black);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'n')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Knight, ChessPieceColor.Black);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'b')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Bishop, ChessPieceColor.Black);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'r')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Rook, ChessPieceColor.Black);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'q')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.Queen, ChessPieceColor.Black);
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == 'k')
                    {
                        Squares[index].Piece = new ChessPiece(ChessPieceType.King, ChessPieceColor.Black);      
                        Squares[index].Piece.Moved = true;
                        index++;
                    }
                    else if (c == '/')
        ...
        internal ChessBoard()
        {
            Squares = new VariableSquare[64];

            for (byte i = 0; i < 64; i++)
            {
                Squares[i] = new VariableSquare();
            }
            ...
            WhiteAttackedSquares = new bool[64];
            BlackAttackedSquares = new bool[64];

        }

        private ChessBoard(VariableSquare[] squares)
        {
            Squares = new VariableSquare[64];
        ...
        //Constructor
        internal ChessBoard(int score) : this()
        {
            Score = score;
        ...
        //Copy Constructor
        internal ChessBoard(ChessBoard board)
        {
            Squares = new VariableSquare[64];
            ...
            for (byte x = 0; x < 64; x++)
            {
                ...
                {
                    Squares[x] = new VariableSquare(board.Squares[x].Piece);
                }
            }
            ...
            for (byte x = 0; x < 64; x++)
            {
                WhiteAttackedSquares[x] = board.WhiteAttackedSquares[x];
                BlackAttackedSquares[x] = board.BlackAttackedSquares[x];
            }
            ...
            LastMove = new ChessMove(board.LastMove);
        ...
        private static bool PromotePawns(ChessBoard board, ChessPiece piece, byte dstPosition, ChessPieceType promoteToPiece)
        {
            if (piece.PieceType == ChessPieceType.Pawn)
            {
                if (dstPosition < 8)
                {
                    ...
                    board.Squares[dstPosition].Piece.PieceValue = ChessPiece.CalculatePieceValue(promoteToPiece);
                    board.Squares[dstPosition].Piece.PieceActionValue = ChessPiece.CalculatePieceActionValue(promoteToPiece);
                    return true;
                }
                if (dstPosition > 55)
                {
        ...
        private static void RecordEnPassant(ChessPieceColor pcColor, ChessPieceType pcType, ChessBoard board, byte srcPosition, byte dstPosition)
        {
        ...
        private static bool SetEnpassantMove(ChessBoard board, byte srcPosition, byte dstPosition, ChessPieceColor pcColor)
        {
            ...
            VariableSquare sqr = board.Squares[dstPosition];
        ...
        private static void KingCastle(ChessBoard board, ChessPiece piece, byte srcPosition, byte dstPosition)
        {
        ...
        //Fast Copy
        internal ChessBoard FastCopy()
        {
            ChessBoard clonedBoard = new ChessBoard(Squares);
        ...
        internal static ChessMove MovePiece(ChessBoard board, byte srcPosition, byte dstPosition, ChessPieceType promoteToPiece)
        {
            ChessPiece piece = board.Squares[srcPosition].Piece;

            //Record my last move
            board.LastMove = new ChessMove();
        ...
        internal static string Fen(bool boardOnly, ChessBoard board)
        {
            ...
            for (byte x = 0; x < 64; x++)
            {
                ...
                if (board.Squares[index].Piece != null)
                {
                    ...
                    {
                        output += ChessPiece.GetPieceTypeShort(board.Squares[index].Piece.PieceType).ToLower();
                    }
                    else
                    {
